...
class GridworldEnv(discrete.DiscreteEnv):
    ...
    def _make(self, plan, rewards):
        ...
        self.start_grid_map = self._read_grid_map(
            self.grid_map_path)  # initial grid map
        self.current_grid_map = np.copy(
            self.start_grid_map)  # current grid map
        self.nbSteps = 0
        self.rstates = {}
        self.P = None
        self.nS = None
    ...
    def _exploreDir(self, gridmap, state, dir, a, b, c):
        ...
        if nstate[0] < gridmap.shape[0] and nstate[0] >= 0 and nstate[
                1] < gridmap.shape[1] and nstate[1] >= 0 and gridmap[
                    nstate[0], nstate[1]] != 1:
    ...
    def _get_agent_pos(self, grid_map):
        state = list(
            map(lambda x: x[0]
                if len(x) > 0 else None, np.where(grid_map == 2)))
        return state
        ...
        if p < 0.2:
            ...
            if action == 0 or action == 1:
                ...
                if p < 0.5:
                    ...
                    action = 1
        npos = (self.currentPos[0] + self.actions[action][0],
                self.currentPos[1] + self.actions[action][1])
        rr = -1 * (self.nbSteps > self.nbMaxSteps)
        if npos[0] >= self.current_grid_map.shape[0] or npos[0] < 0 or npos[
                1] >= self.current_grid_map.shape[1] or npos[
                    1] < 0 or self.current_grid_map[npos[0], npos[1]] == 1:
            return (self.current_grid_map, self.rewards[0] + rr,
